...
(function() {
  var BOM, BOOL, CALLABLE, CODE, COFFEE_ALIASES, COFFEE_ALIAS_MAP, COFFEE_KEYWORDS, COMMENT, COMPARE, COMPOUND_ASSIGN, HERECOMMENT_ILLEGAL, HEREDOC_DOUBLE, HEREDOC_INDENT, HEREDOC_SINGLE, HEREGEX, HEREGEX_OMIT, IDENTIFIER, INDENTABLE_CLOSERS, INDEXABLE, INVALID_ESCAPE, INVERSES, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LEADING_BLANK_LINE, LINE_BREAK, LINE_CONTINUER, LITDOC, LOGIC, Lexer, MATH, MULTI_DENT, NOT_REGEX, NUMBER, OPERATOR, POSSIBLY_DIVISION, REGEX, REGEX_FLAGS, REGEX_ILLEGAL, RELATION, RESERVED, Rewriter, SHIFT, SIMPLE_STRING_OMIT, STRICT_PROSCRIBED, STRING_DOUBLE, STRING_OMIT, STRING_SINGLE, STRING_START, TRAILING_BLANK_LINE, TRAILING_SPACES, UNARY, UNARY_MATH, VALID_FLAGS, WHITESPACE, compact, count, invertLiterate, key, locationDataToString, ref, ref1, repeat, starts, throwSyntaxError,
  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.doTokenize = function(code, opts) {
      ...
      while (this.chunk = code.slice(i)) {
        ...
        consumed = this.identifierToken() || this.commentToken() || this.whitespaceToken() || this.litdocToken() || this.lineToken() || this.stringToken() || this.numberToken() || this.regexToken() || this.jsToken() || this.literalToken();
    ...
    Lexer.prototype.clone = function() {
      var newLexer;
      newLexer = new Lexer;
      newLexer.Compiler = this.Compiler;
      return newLexer;
    };

    Lexer.prototype.tokenize = function(code, opts) {
      var compiler;
      compiler = this.compiler != null ? this.compiler : this.compiler = new this.Compiler;
      compiler.lexer = this;
      compiler.code = code;
      ...
      compiler.code = code;
      return this.applyReify(compiler.lexer, function() {
        return this.doTokenize(code, opts);
      });
    };

    Lexer.prototype.applyReify = function(val, cb) {
      var ref2, th;
      if ((val != null) && ((ref2 = typeof val) === 'object' || ref2 === 'function') && ((th = val.then) != null) && typeof th === 'function') {
        return th.call(val, function(res) {
          return cb.call(res);
        });
      } else {
        return cb.call(val);
    ...
    Lexer.prototype.processDirectives = function(code) {
      var m;
      m = /(^|\n)@@@@(.*\r?(:?\n|$)(?:(:?\r?\n)*(?:\s.*\r?(:?\n|$))+)*)/g;
      code = code.replace(m, (function(_this) {
        return function(m, start, line) {
          var compiler;
          compiler = _this.compiler;
          compiler["eval"]('@>' + line.replace(/^\S/, function(m) {
            return ' @lexer.' + m;
          })).call(compiler);
          return start + line.replace(/.+/g, function(x) {
            return '# Processed: ' + x;
          });
        };
      })(this));
    ...
    Lexer.prototype.identifierToken = function() {
      var alias, colon, colonOffset, forcedIdentifier, id, idLength, input, match, poppedToken, prev, ref2, ref3, ref4, ref5, ref6, tag, tagToken;
      ...
      forcedIdentifier = colon || (prev != null) && (((ref3 = prev[0]) === '.' || ref3 === '?.' || ref3 === '::' || ref3 === '?::') || !prev.spaced && ((ref4 = prev[0]) === '@@' || ref4 === '@'));
      tag = 'IDENTIFIER';
      if (!forcedIdentifier && (indexOf.call(JS_KEYWORDS, id) >= 0 || indexOf.call(COFFEE_KEYWORDS, id) >= 0)) {
        ...
        } else if (tag === 'WHEN' && (ref5 = this.tag(), indexOf.call(LINE_BREAK, ref5) >= 0)) {
          tag = 'LEADING_WHEN';
        } else if (tag === 'FOR') {
          this.seenFor = true;
        } else if (tag === 'UNLESS') {
    ...
    Lexer.prototype.litdocToken = function() {
      var indent, length, match, ref2, string, theLastToken;
      ...
      string = string.replace(/\"/g, "\\\"");
      string = '"' + string + '"';
      ref2 = this.tokens, theLastToken = ref2[ref2.length - 1];
      if (theLastToken != null) {
    ...
    Lexer.prototype.addImplicitStart = function(tag) {
      var i, ref2, ref3, stack, tok, tokens, ttt;
      stack = [];
      tokens = this.tokens;
      i = tokens.length;
      if (i <= 0) {
      ...
      if ((ref2 = (ref3 = tokens[i - 1]) != null ? ref3[0] : void 0) === 'TERMINATOR') {
        return;
      }
      while (tok = tokens[--i]) {
        ttt = tok[0];
    ...
    Lexer.prototype.matchWithInterpolations = function(regex, delimiter) {
      var close, column, firstToken, index, lastToken, line, nested, offsetInChunk, open, outerLexer, ref2, ref3, ref4, str, strPart, tokens;
      outerLexer = this;
      tokens = [];
      ...
      while (true) {
        ...
        ref3 = outerLexer.clone().doTokenize(str.slice(1), {
          line: line,
          column: column,
          untilBalanced: true
      ...
      body = body.replace(regex, function(match, backslash, nul, delimiter, lf, cr, ls, ps, other) {
        switch (false) {
          ...
          case !ps:
            return '\\u2029';
          case !other:
            return (options.double ? "\\" + other : other);
        }
      });
