  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.tokenize = function(code, opts) {
      ...
      while (this.chunk = code.slice(i)) {
        consumed = this.identifierToken() || this.commentToken() || this.whitespaceToken() || this.lineToken() || this.literalBlock() || this.heredocToken() || this.stringToken() || this.numberToken() || this.regexToken() || this.jsToken() || this.literalToken();
    ...
    Lexer.prototype.literalBlock = function() {
      var block, end, match, start, string;
      if (!(match = LITBLOCK.exec(this.chunk))) {
        return 0;
      }
      if (this.chunkColumn !== this.indent) {
        this.error("Literal block should start on its own line");
      }
      if ((end = RegExp("\\n[\\t ]{0," + (this.indent - 1) + "}\\S").exec(this.chunk))) {
        end = end.index;
      } else {
        end = this.chunk.length;
      }
      if (!(start = /^''''[ \t]*\n/.exec(this.chunk))) {
        this.error("Literal string block should start with a new line: " + this.chunk);
      }
      start = start[0].length - 1;
      block = this.chunk.substring(start, end);
      string = block.replace(RegExp("\\n[\\t ]{" + this.indent + "}", "g"), "\n");
      string = string.substring(1);
      string = string.replace(/\\/g, "\\\\");
      string = string.replace(/\n/g, "\\n");
      string = string.replace(/\"/g, "\\\"");
      string = '"' + string + '"';
      this.token('STRING', string, 0, block.length + start);
      return block.length + start;
    };
